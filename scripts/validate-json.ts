...
const validateJson = (filepath: string) => {
  try {
    ...
    console.log(
      `✅ Great! Successfully validated the JSON file at ${filepath}`,
    );
  } catch (err) {
    console.error(
      `👹 Oops! Failed to validate the JSON file at ${filepath}. Error:`,
      err,
    );
    throw err;
  }
...
const validateJsonFilesInDirectory = ({ dirPath }: { dirPath: string }) => {
