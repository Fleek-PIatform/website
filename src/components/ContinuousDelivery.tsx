...
const ContinuousDelivery = () => (
  <Container>
    <PageSection>
      <GridLayout className="gap-y-20 py-40 lg:py-80">
        <img
          src={imgRepoImage}
          alt="Fleek Site Preview"
          className="col-span-6 col-start-10 hidden lg:block"
          // TODO: check what the original ideia was for blur
          // as this block was a nextjs img component
          // placeholder="blur"
        />
        <div className="col-span-16 flex flex-col gap-48 lg:col-span-9 lg:col-start-1 lg:max-w-[670px]">
          <div className="flex flex-col gap-24 text-center lg:text-start">
            ...
            <Text className="text-ui-light-grey">
              Whether you want Fleek to handle the entire build and deployment,
              or you want a custom CI/CD flow, we got you.
            </Text>
          </div>
          <div className="flex flex-col gap-20 px-10 lg:flex-row">
            {CICDFeatures.map((feature) => (
              <TransparentCard key={feature.description} {...feature}>
                <Text
                  style="caption-s"
                  className="col-span-3 whitespace-pre-line"
                >
                  {feature.description}
                </Text>
              </TransparentCard>
