    ...
    <PageSection>
      <GridLayout className="px-16 py-50 lg:py-80 lg:pl-48">
        <img
          ...
          // placeholder="blur"
        />
        <div className="flex flex-col gap-30 col-span-16 text-center lg:text-start lg:col-span-8">
          <div className="flex flex-col gap-15">
            <TextGlowHoverEffect style="h4">
              <Text style="h4">Web3 Storage, Done Right</Text>
            </TextGlowHoverEffect>
            <Text className="text-ui-light-grey">Fleek takes pride in pushing the limits of decentralized storage. We use a best in class decentralized stack. and we continuously optimize it to be both maximally decentralized and performant.</Text>
          </div>
          <div className="flex flex-col w-full text-center lg:text-start lg:w-fit gap-8">
            {FEATURES.map(({ label, icon }) => (
              <div key={label} className="rounded-12 bg-gradient-to-b from-[rgba(75,75,75,0.8)] to-[rgba(75,75,75,0.2)] p-1">
                <div className="flex self-stretch items-center gap-16 rounded-11 bg-ui-black py-12 pl-24 pr-32 shadow-dark">
                  <img {...icon} width={24} height={24} /> 
                  <Text style="caption-s" className="w-full">{label}</Text>
                </div>
              </div>
            ))}
          </div>
