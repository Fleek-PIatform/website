...
Here, we have created a new file called `function.js` that has a single export, the function `main`. Note the use of the params object, which will include the request data sent whenever this function is called.
...
Additionally, you can pass a `--env-file` flag to the CLI, followed by a file location relative to the current directory, in order to load environment variables from a file. This file should represent variables using a `<variable>=value` syntax or <variable> (setting the value from the local environment), and should use `#` for comments. Lines beginning with `#` are considered line comments and are ignored, whereas a `#` appearing anywhere else in a line is considered a part of the variable value. In the example below, the user's environment variables are located in an `env.list` file.
...
Lastly, you can use variables exported to your local environment: If you have exported a variable to your environment with export VAR1='foo'andexport VAR2='bar', for example, you can run:
