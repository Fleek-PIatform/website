...
5. Go through the steps (from step 1 through to step 4) and follow the instructions per step. The steps take you from creating the Fleek Function which will connect with the Fleek Function on the platform and gets you to deploy from your machine to the function you already created from your platform.
   ...
   - After installation, type the following command to login to your Fleek account from the CLI:
...
You have successfully deployed a Fleek Function using the above steps from the Fleek platform UI and now the first section of our Fleek Function "Overview" should look like this now with the "Pending" status updated:
...
If you back to the "Functions" page, you should see a list of all your functions with the "Active" state for the deployed ones and "Not deployed" for the ones that have not been deployed.

## Managing Fleek Functions

Fleek Functions can be managed directly from the Fleek platform within a project now. Most of the operations that required using the Fleek CLI before can
now from the Fleek UI.
...
Fleek Functions can be deleted from the settings as well by scrolling to the bottom-most section of the settings page and clicking on the "Delete Function" button.

It brings up a modal that prompts the user to enter the Fleek Function name and the confirm with the "Delete Function" button that is active when the name the user enters matches that in the modal window.
